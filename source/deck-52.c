...
pack_value_t calc_holdem_5(unsigned int n, const input_t * path)
{
    if (n != 5) {
        fprintf(stderr, "Assertion failed: calc_holdem_5 requires n = 5, but %u as passed.\n", n);
    }
    card_t cards[5];
...
void build_holdem_5(void * script)
{
    ...
    script_step_pack(script, calc_holdem_5, 0);
...
int check_holdem_5(const void * ofsm)
{
    ...
    save_binary("holdem-5.bin", "OFSM Holdem 5", &array);
